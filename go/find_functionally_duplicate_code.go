...
func areParameterTypeCountsEqual(countsI, countsJ map[string]int) bool {
...
// TODO: compare return types + test case
		...
		for j, fileJ := range files[i:] {
			for k, functionI := range fileI.functions {
				for l, functionJ := range fileJ.functions {
					...
					if areParameterTypeCountsEqual(functionI.parameterTypeCount, functionJ.parameterTypeCount) &&
